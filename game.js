...
function handleResult(finalRotation) {
    ...
    // Check range bets
    if (resultInfo.range === 'low' && state.bets['1-18']) {
        winnings += state.bets['1-18'] * 2;
    } else if (resultInfo.range === 'high' && state.bets['19-36']) {
        winnings += state.bets['19-36'] * 2;
    }
    // Check dozen bets
